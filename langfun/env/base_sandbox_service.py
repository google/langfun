...
"""Common bases for sandbox services."""
...
class BaseSandboxService(interface.SandboxService):
  """Common base for sandbox services that support pooling."""
  ...
  def _create_sandbox(
    ...
    Raises:
      interface.SandboxServiceError: If service cannot create the sandbox.
  ...
  def _start(self) -> None:
    """Implementation of starting the service."""
    self._housekeep_event.clear()
  ...
  def _shutdown(self) -> None:
    """Implementation of shutting down the service."""
  ...
  def _acquire(
    ...
    """Implementation of acquiring a sandbox from the service."""
  ...
  def _housekeep_cycle(self) -> tuple[bool, dict[str, Any]]:
    """Implementation of a single housekeeping cycle.

    Returns:
      A tuple (housekeep_succeeded, housekeep_output).
    """
    return True, dict()
  ...
  def _check_feature_requirements(self) -> None:
    ...
    for name, feature in self.features.items():
      if not feature.is_sandbox_based:
        raise ValueError(
            f'Feature {name!r} is not sandbox-based. '
            'Sandbox service only supports sandbox-based features.'
        )
  ...
  def id(self) -> interface.SandboxService.Id:
    """Returns the root directory for the sandbox service."""
    if self.environment is None:
      return interface.SandboxService.Id(
          None, pg.utils.camel_to_snake(self.__class__.__name__)
      )
    return interface.SandboxService.Id(
        self.environment.id if self.environment else None, self.name
    )

  @property
  def start_time(self) -> float | None:
    """Returns the start time of the sandbox service."""
    return self._start_time

  @property
  def offline_duration(self) -> float:
    """Returns the offline duration of the sandbox service."""
  ...
  def status(self) -> Status:
    """Returns sandbox service status."""
    return self._status

  def _set_status(self, status: Status) -> None:
    """Sets the status of the sandbox service."""
    self._status = status
  ...
  def start(self) -> None:
    ...
    Raises:
      interface.SandboxServiceOutageError: If the sandbox service is out of
        service.
    """
    assert self._status == self.Status.CREATED, (
        f'Sandbox service {self.id} cannot be started because '
  ...
  def acquire(
    ...
    Args:
      image_id: The image ID to use for the sandbox. If None, it will be
        automatically determined by the sandbox service.

    Returns:
      The acquired sandbox.

    Raises:
      interface.SandboxServiceOutageError: If the sandbox service is offline and
    ...
    if image_id is None:
      if not self.image_ids:
        raise ValueError(
            f'Sandbox service {self.id} does not have a default image ID. '
      ...
      raise ValueError(
          f'Sandbox service {self.id} does not serve image ID {image_id!r}. '
  ...
  def _bring_up_sandbox_with_retry(
    ...
    Args:
      ...
      reusable: If True, the sandbox will be marked as reusable.
      shutdown_env_upon_outage: Whether to shutdown the sandbox service when the
  ...
  def _housekeep_loop(self) -> None:
    ...
    while self._status not in (self.Status.SHUTTING_DOWN, self.Status.OFFLINE):
      ...
      is_online, housekeep_output = self._housekeep_cycle()
      ...
      if is_online:
        ...
        self.on_housekeep(
            duration=time.time() - housekeep_start_time,
            error=interface.SandboxServiceOutageError(
                sandbox_service=self, offline_duration=self.offline_duration
            ),
            **housekeep_output
        )
        self.shutdown()
  ...
  def on_starting(self) -> None:
    """Called when the sandbox service is getting started."""
    self.event_handler.on_sandbox_service_starting(self)

  def on_start(
      self,
      duration: float, error: BaseException | None = None
  ) -> None:
    """Called when the sandbox service is started."""
    self.event_handler.on_sandbox_service_start(self, duration, error)
  ...
  def on_housekeep(
    ...
    """Called when the sandbox service finishes a round of housekeeping."""
    self.event_handler.on_sandbox_service_housekeep(
        self, self.housekeep_counter, duration, error, **kwargs
    )

  def on_shutting_down(self) -> None:
    """Called when the sandbox service is shutting down."""
    self.event_handler.on_sandbox_service_shutting_down(
        self, self.offline_duration
    )
  ...
  def on_shutdown(
    ...
    """Called when the sandbox service is shutdown."""
    lifetime = (time.time() - self.start_time) if self.start_time else 0.0
    self.event_handler.on_sandbox_service_shutdown(
        self, duration, lifetime, error
    )


class PooledSandboxService(BaseSandboxService):
  """Common base for sandbox services that support pooling."""
  ...
  load_balancer: Annotated[
      ...
      (
          'The load balancer for the sandbox service to acquire sandboxes.'
      )
  ...
  def _on_bound(self) -> None:
    self._sandbox_pool: dict[str, list[interface.Sandbox]] = (
  ...
  def stats(self) -> dict[str, Any]:
    """Returns the stats of the sandbox service."""
    stats_by_image_id = {}
  ...
  @property
  def sandbox_pool(self) -> dict[str, list[interface.Sandbox]]:
    """Returns the sandbox pool."""
    return self._sandbox_pool

  def enable_pooling(self, image_id: str) -> bool:
    """Returns whether the sandbox service enables pooling."""
  ...
  #
  # Sandbox service operations.
  #

  def _start(self) -> None:
    """Implementation of starting the sandbox service."""
  ...
  def _shutdown(self) -> None:
    """Implementation of shutting down the sandbox service."""
    super()._shutdown()
  ...
  def _housekeep_cycle(self) -> tuple[bool, dict[str, Any]]:
    """Implementation of a housekeeping cycle for the sandbox service."""
  ...
  def _replace_dead_sandboxes(
      self,
      dead_sandbox_entries: list[tuple[str, int, interface.Sandbox]]
  ) -> dict[str, list[int]]:
    ...
    def _replace(sandbox_entry: tuple[str, int, interface.Sandbox]):
