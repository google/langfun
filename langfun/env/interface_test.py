...
class IdTest(unittest.TestCase):
  ...
  def test_environment_id(self):
    ...
    self.assertEqual(env_id.working_dir(root_dir='/tmp'), '/tmp/env_1/ab_c_def')
  ...
  def test_sandbox_service_id(self):
    sandbox_service_id = interface.SandboxService.Id(
        environment_id=interface.AbstractEnvironment.Id('env'), name='service'
    )
    self.assertEqual(str(sandbox_service_id), 'env/service')
    self.assertEqual(
        sandbox_service_id.working_dir(root_dir='/tmp'), '/tmp/env/service'
    )
  ...
  def test_sandbox_id(self):
    sandbox_id = interface.Sandbox.Id(
        service_id=interface.SandboxService.Id(
            interface.AbstractEnvironment.Id('env'), name='docker'
        ),
        image_id='image:2025_01_01_00_00_00',
        sandbox_id='sandbox',
    )
    self.assertEqual(
        str(sandbox_id), 'env/docker/image:2025_01_01_00_00_00:sandbox'
    )
    self.assertEqual(
  ...
  def test_feature_id(self):
    ...
    self.assertEqual(
        feature_id.working_dir(root_dir='/tmp'), '/tmp/env/feature'
    )
    ...
    feature_id = interface.Feature.Id(container_id=None, feature_name='feature')
    self.assertEqual(str(feature_id), 'feature')
    self.assertEqual(feature_id.working_dir(root_dir='/tmp'), '/tmp/feature')
...
class TestingSandbox(interface.Sandbox):
  ...
  id = interface.Sandbox.Id(
      service_id=interface.SandboxService.Id(
          interface.AbstractEnvironment.Id('env'), name='test_sandbox_service'
      ),
      image_id='test_image',
      sandbox_id='0:0',
  )
  ...
  def track_activity(
      self, name: str, feature: interface.Feature | None = None, **kwargs
  ) -> Iterator[None]:
    error = None
    try:
      yield
...
class InterfaceTests(unittest.TestCase):
  ...
  def test_abstract_environment_get_sandbox_service(self):
    ...
    # 1. Match by name
    self.assertIs(env._get_sandbox_service(sandbox_service='ss'), mock_service)
    ...
    # 3.5. Default image ID (None) returns the first service
    self.assertIs(env._get_sandbox_service(), mock_service)

    # 4. Error: image ID not served, and no dynamic sandbox service
    static_env = BareEnvironment(
        id='env2',
        sandboxes={'ss': mock_service},
    )
    with self.assertRaisesRegex(ValueError, "does not serve image ID 'img2'"):
      static_env._get_sandbox_service(image_id='img2')
