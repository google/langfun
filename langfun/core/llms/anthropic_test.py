...
# `lf` does not re-export `RetryError`, which is what a caller actually
# receives once the retry wrapper is in the path -- even at max_attempts=1.
...
def _sse_stream(content_events: str, stop_reason: str) -> str:
  """Wraps content events in the message_start/message_delta/stop envelope."""
  return (
      'event: message_start\n'
      'data: {"type":"message_start","message":{"id":"m","type":"message",'
...
_THINKING_ONLY_EVENTS = (
    'event: content_block_start\n'
    'data: {"type":"content_block_start","index":0,'
    '"content_block":{"type":"thinking","thinking":""}}\n\n'
    'event: content_block_delta\n'
    'data: {"type":"content_block_delta","index":0,'
    '"delta":{"type":"thinking_delta","thinking":"Weighing the options and"}}'
    '\n\n'
    'event: content_block_stop\n'
    'data: {"type":"content_block_stop","index":0}\n\n'
)
...
class EmptyGenerationDiagnosticsTest(unittest.TestCase):
  """A text-less 200 must explain itself from the error alone.

  Three different provider behaviors all converge on `message.text == ''`:
  thinking that consumed the whole budget, a refusal, and a content list that
  is simply empty. They need different fixes, so an error that names only the
  model (the generic check in `LanguageModel.sample()`) cannot be acted on --
  and because `RetryError` keeps only the final attempt, one contentless line
  is all a caller ever sees. These tests pin the evidence that makes the three
  distinguishable.
  """

  def _call_and_capture(self, sse: str) -> str:
    """Runs a full streamed call and returns the underlying error's message.

    The call raises `RetryError`, not `EmptyGenerationError`, even at
    `max_attempts=1`: the retry wrapper is unconditionally in the path. Since
    `RetryError.__str__` keeps only the final attempt, the per-attempt errors on
    `.errors` are the only place the diagnosis survives -- so the test reads it
    from exactly where a caller would have to.

    Args:
      sse: The Server-Sent Events stream to feed as the response.

    Returns:
      The string message of the underlying `EmptyGenerationError`.
    """

    def _post_with_request(url=None, **kwargs):
      # `mock_sse_post` builds a bare `requests.Response`, which has no
      # `.request` -- but the request body is where `sent_max_tokens` and the
      # thinking config come from, so a mock without it cannot exercise the
      # request side of the report at all.
      response = mock_sse_post(sse, chunk_size_bytes=64)(url, **kwargs)
      prepared = requests.PreparedRequest()
      prepared.body = json_lib.dumps(kwargs.get('json'))
      response.request = prepared
      return response

    with mock.patch('requests.Session.post') as mock_request:
      mock_request.side_effect = _post_with_request
      lm = anthropic.Claude5Opus(api_key='fake_key', max_attempts=1)
      with self.assertRaises(lf_concurrent.RetryError) as ctx:
        lm('hello')
      last_error = ctx.exception.errors[-1]
      self.assertIsInstance(last_error, lf.EmptyGenerationError)
      return str(last_error)

  def test_thinking_only_response_reports_stop_reason_and_thinking_tail(self):
    """Thinking-only output must surface `stop_reason` AND the thinking tail.

    `stop_reason` says generation stopped on the output cap; the thinking tail
    says whether the model was cut off mid-thought (budget exhaustion) or ended
    cleanly and simply never wrote text (a different root cause needing a
    different fix). Neither is recoverable from the response afterwards, so
    both must be captured at the point of failure.
    """
    report = self._call_and_capture(
        _sse_stream(_THINKING_ONLY_EVENTS, 'max_tokens')
    )
    self.assertIn('[EMPTY_GENERATION]', report)
    self.assertIn("stop_reason='max_tokens'", report)
    # Block inventory proves the response was thinking-only, not empty.
    self.assertIn('thinking:24', report)
    self.assertIn('output_tokens=9999', report)
    # The request side is the denominator that makes output_tokens meaningful.
    self.assertIn('sent_max_tokens=128000', report)
    self.assertIn("thinking_tail='Weighing the options and'", report)

  def test_refusal_is_distinguishable_from_budget_exhaustion(self):
    """The same empty text under a different `stop_reason` must read differently."""
    report = self._call_and_capture(
        _sse_stream(_THINKING_ONLY_EVENTS, 'refusal')
    )
    self.assertIn("stop_reason='refusal'", report)
    # The point is discrimination: identical empty text, different verdict.
    self.assertNotIn("stop_reason='max_tokens'", report)

  def test_empty_content_list_reports_no_blocks(self):
    """A response with no content blocks must not look like a thinking-only one."""
    report = self._call_and_capture(_sse_stream('', 'end_turn'))
    self.assertIn('blocks=[none]', report)
    self.assertNotIn('thinking_tail', report)

  def test_raw_body_is_dumped_for_unanticipated_shapes(self):
    """The dump is the escape hatch for a cause no extracted field covers."""
    report = self._call_and_capture(
        _sse_stream(_THINKING_ONLY_EVENTS, 'max_tokens')
    )
    match = re.search(r'raw_body_dump=(\S+)', report)
    self.assertIsNotNone(match)
    dump_path = match.group(1)
    self.assertTrue(os.path.exists(dump_path), dump_path)
    with open(dump_path) as f:
      dumped = f.read()
    # The dump is the body itself, not a summary of it.
    self.assertIn('thinking_delta', dumped)
    os.remove(dump_path)

  def test_successful_call_is_unaffected_and_leaks_no_diagnostics(self):
    """No regression: a normal response still parses, with no diagnostics leak.

    The forensics ride through `result()` inside the message dict, so the
    guarantee worth pinning is that they never reach the returned message or
    ...
    with mock.patch('requests.Session.post') as mock_request:
      ...
      self.assertEqual(response.text, 'Hello, world!')
      self.assertEqual(response.usage.completion_tokens, 42)
      self.assertNotIn('_lf_diagnostics', str(response))

  def test_result_without_diagnostics_still_reports(self):
    """`result()` called directly (no HTTP layer) must degrade, not crash."""
    lm = anthropic.Claude5Opus(api_key='fake_key')
    with self.assertRaises(lf.EmptyGenerationError) as ctx:
      lm.result({
          'role': 'assistant',
          'content': [],
          'stop_reason': 'end_turn',
    ...
    self.assertIn("stop_reason='end_turn'", report)
    self.assertIn('sent_max_tokens=None', report)
    self.assertIn('raw_body_dump=(unavailable)', report)
