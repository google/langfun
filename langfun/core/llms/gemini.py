...
class GeminiEmptyGenerationError(lf.LMError):
  ...
  Two response signatures produce an answer-less candidate as an EXPECTED
  ...
  * ``finishReason == 'STOP'`` with ``candidatesTokenCount == 0`` -- the model
  ...
  Both are reproduced by re-sending the same request, so this subclasses the
  NON-retryable ``lf.LMError`` -- deliberately NOT ``lf.EmptyGenerationError``
  / ``lf.RetryableLMError``. Otherwise the empty text reaches
  ``LanguageModel.sample()``, which raises the retryable
  ``lf.EmptyGenerationError`` and re-sends the full prompt up to
  ``max_attempts`` times, burning quota and latency only to fail with an
  opaque error. Mirrors ``beyond_api.BeyondThinkingBudgetExhaustedError``.
  """
...
SUPPORTED_MODELS = [
    ...
    GeminiModelInfo(
        ...
        # https://ai.google.dev/gemini-api/docs/changelog#03-03-2026
        release_date=datetime.datetime(2026, 3, 3),
        input_modalities=GeminiModelInfo.ALL_SUPPORTED_INPUT_TYPES,
        context_length=lf.ModelInfo.ContextLength(
            max_input_tokens=1_048_576,
            max_output_tokens=65_536,
...
class Gemini(rest.REST):
  ...
  def _generation_config(
    ...
    if (
        self.model_id.startswith('gemini-3')
        and 'lite' not in self.model_id
        and not (
  ...
  def _check_answer_present(
    ...
    """Fails fast when a candidate carries no answer, deterministically.

    Only the two signatures documented on `GeminiEmptyGenerationError` are
    reclassified. Every other answer-less case (an unrelated `finishReason`, a
    function-call-only candidate that did emit tokens, a missing
    `finishReason`) is left alone so the pre-existing retryable
    `lf.EmptyGenerationError` path keeps handling it -- those may genuinely be
    transient.
    ...
    raise GeminiEmptyGenerationError(
