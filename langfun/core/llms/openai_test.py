...
def mock_completion_query(prompt, *, n=1, **kwargs):
  ...
  return pg.Dict(choices=choices)
...
class OpenaiTest(unittest.TestCase):
  ...
  def test_call(self):
    with mock.patch('openai.Completion.create') as mock_completion:
      ...
      lm = openai.OpenAI(api_key='test_key')
      self.assertEqual(
  ...
  def test_sample(self):
    ...
    self.assertEqual(results[0], lf.LMSamplingResult([
        lf.LMSample(text='Sample 0 for prompt 0.', score=0.0),
        lf.LMSample(text='Sample 1 for prompt 0.', score=0.1),
        lf.LMSample(text='Sample 2 for prompt 0.', score=0.2),
    ]))
    self.assertEqual(results[1], lf.LMSamplingResult([
        lf.LMSample(text='Sample 0 for prompt 1.', score=0.0),
        lf.LMSample(text='Sample 1 for prompt 1.', score=0.1),
        lf.LMSample(text='Sample 2 for prompt 1.', score=0.2),
    ]))
