...
if beam is not None:
  class _EvaluateFn(beam.DoFn):
    ...
    def setup(self):
      ...
      self._evaluation = pg.from_json_str(self._evaluation_str)
      self._evaluation.setup()

    def teardown(self):
      assert self._evaluation is not None
      self._evaluation.teardown()
    ...
    def process(
      ...
      example = self._evaluation.evaluate(pg.from_json_str(example_json))
      ...
      if self._generate_html:
        try:
          ...
          html.save(
              os.path.join(self._output_dir, f'{example.id}.html')
          )
        except BaseException as e:  # pylint: disable=broad-except
          pg.logging.error(
              '[%s] Failed to generate HTML for example %d: %s',
              self._evaluation.id,
              example.id,
...
class BeamRunner(base.RunnerBase):
  ...
  def _on_bound(self):
    ...
    host_plugins = []
    generate_example_html = False
    for plugin in self.plugins:
      if isinstance(plugin, reporting.ExampleHtmlGenerator):
        generate_example_html = True
      elif isinstance(plugin, checkpointing.Checkpointer):
        pg.logging.warning(
    ...
    self._generate_example_html = generate_example_html
    super()._on_bound()
  ...
  def _run(self, evaluations: list[evaluation_lib.Evaluation]) -> None:
    ...
    with beam.Pipeline(
      ...
      for i, evaluation in enumerate(evaluations):
        ...
        evaluation_str = pg.to_json_str(evaluation)
            ...
            >> beam.ParDo(
                _EvaluateFn(
                    ...
                    generate_example_html=(
                        self._generate_example_html
                        and self.current_run.generate_example_html
                    ),
