...
class BeamRunnerTest(RunnerTest):

  def test_basic(self):
    plugin = eval_test_helper.TestPlugin()
    exp = eval_test_helper.test_experiment()
    root_dir = os.path.join(tempfile.mkdtemp(), 'test_beam_runner')
    run = exp.run(
        root_dir,
        runner='beam',
        plugins=[
  ...
  def test_collect_ckpt_failure(self):
    ...
    plugin = MyPlugin()
    exp = eval_test_helper.test_experiment()
    root_dir = os.path.join(tempfile.mkdtemp(), 'test_collect_ckpt_failure')
    run = exp.run(
        root_dir,
        runner='beam',
        plugins=[plugin],
  ...
  def test_beam_runner_does_not_support_cache(self):
    exp = eval_test_helper.test_experiment()
    root_dir = os.path.join(tempfile.mkdtemp(), 'test_beam_runner_cache')
  ...
  def test_no_beam(self):
    ...
    with self.assertRaisesRegex(ValueError, 'Beam is not installed'):
      ...
      root_dir = os.path.join(tempfile.mkdtemp(), 'test_no_beam')
