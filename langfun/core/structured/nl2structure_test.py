...
class ParseStructureTest(unittest.TestCase):
  ...
  def test_render_no_examples(self):
    l = nl2structure.ParseStructure(int)
  ...
  def test_transform(self):
    ...
    with lf.context(
      ...
      l = lf.LangFunc(lm_input) >> nl2structure.ParseStructure(
          [Itinerary],
          examples=[
              mapping.MappingExample(
...
class QueryStructureTest(unittest.TestCase):
  ...
  def test_render_no_examples(self):
    l = nl2structure.QueryStructure(int)
  ...
  def test_bad_transform(self):
    with lf.context(
        lm=fake.StaticSequence(['3'], debug=True),
        override_attrs=True,
    ):
      with self.assertRaisesRegex(
          mapping.MappingError,
